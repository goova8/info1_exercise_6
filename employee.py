...
class Manager(Employee):
    ...
    def get_sales_report(self,salesman):
        try:
            print("{}'s current cumulative sales:\n{}".format(salesman.get_name(), Salesman.sales[salesman.get_name()]))
        except:
            raise KeyError("{} does not have any sales yet.".format(salesman.get_name()))
...
class Salesman(Employee):
    ...
    def sale(self,vehicle,sales_price,customer):
        if customer.credit_score() == True:
            if self.get_name() not in Salesman.sales:
